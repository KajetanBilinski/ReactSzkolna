...
import {deleteSzkolniaktoPotrawaApiCall} from "../../apiCalls/szkolniakApiCalls";
...
class SzkolniakAddAfera extends React.Component
{
    ...
    componentDidMount()
    {
        this.fetchSzkolPotrawaDelete();
    }

    fetchSzkolPotrawaDelete = () =>
    {
        deleteSzkolniaktoPotrawaApiCall(this.state.szkolId,this.state.idPotrawa).then(res=>res.json()).then((data)=>
            {
                console.log(data);
                if(data.message)
                {
    ...
    render()
    {
        ...
        {
            content = <p>Trwa usuwanie potrawy szkolniaka</p>
        }
        else
        {
            content = <p>{message}</p>
