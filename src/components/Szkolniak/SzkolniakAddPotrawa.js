...
import { addSzkolniaktoPotrawaApiCall} from "../../apiCalls/szkolniakApiCalls";
...
class SzkolniakAddAfera extends React.Component
{
    ...
    componentDidMount()
    {
        this.fetchSzkolPotrawaAdd();
    }

    fetchSzkolPotrawaAdd = () =>
    {
        addSzkolniaktoPotrawaApiCall(this.state.szkolId,this.state.idPotrawa).then(res=>res.json()).then((data)=>
            {
                console.log(data);
                if(data.message)
                {
    ...
    render()
    {
        ...
        {
            content = <p>Trwa dodawanie potrawy szkolniaka</p>
        }
        else
        {
            content = <p>{message}</p>
